...
class Help(commands.Cog):
    ...
    async def help(self, ctx, arg=None):
        if arg is None:
            ...
            math_embed.add_field(name="```^fetchamc <year> <contest version> <problem number>```", value="fetches any AIME problem from any given year", inline=False)
            math_embed.add_field(name="```^fetchaime <year> <contest version> <problem number>```", value="fetches any AIME problem from any given year", inline=False)
            math_embed.add_field(name="```^fetchusamo <year> <problem number>```", value="fetches any USAMO problem from any given year", inline=False)
            math_embed.add_field(name="```^fetchusajmo <year> <problem number>```", value="fetches any USAJMO problem from any given year", inline=False)
            ...
            math_embed2.add_field(name="```^stats```", value="gives you your statistics. Type `^help statistics` for more info")
            math_embed2.add_field(name="```^last5 <year>```", value="returns the last 5 of any AMC contest (Ranked)")
            math_embed2.add_field(name="```^amc10 <diffculty level>```", value="gives any AMC10 problem of that difficulty (Ranked)")
            math_embed2.add_field(name="```^amc12 <diffculty level>```", value="gives any AMC12 problem of that difficulty (Ranked)")
            math_embed2.add_field(name="```^random <contest name>```", value="fetches any problem from a specified contest (Ranked)")
